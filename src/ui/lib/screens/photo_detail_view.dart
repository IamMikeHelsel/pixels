...
class _PhotoDetailViewState extends State<PhotoDetailView> {
  ...
  Future<void> _updateRating(int rating) async {
    ...
    try {
      final success = await _backendService.updatePhotoRating(
        _photo!.id, 
        rating
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: _photo != null 
            ? Text(_photo!.fileName)
            : const Text('Photo Detail'),
        actions: [
          if (_photo != null) ...[
            // Favorite toggle
            IconButton(
              ...
              tooltip: _photo!.isFavorite ? 'Remove from favorites' : 'Add to favorites',
            ),
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              _buildInfoRow('Dimensions', '${_photo!.width} × ${_photo!.height}'),
